...
(deftest sheet-has-the-native-modal-surface-contract
  (let [batch
        (record proto/patch-batch
                (generation 1)
                (ops [(proto/create-node-op 2 proto/Sheet)
                      (proto/set-prop-op
    ...
    (doseq [property [proto/Gap proto/MainAlignment proto/CrossAlignment
                      proto/Selected proto/Enabled]]
      (is (not (proto/property-supported? proto/Sheet property))
          "sheet rejects flow and control state"))
    (is (proto/can-contain-children? proto/Sheet)
        "sheet retains stacking content")
    (is (proto/event-supported? proto/Sheet (proto/Dismiss 1))
        "sheet reports native dismissal")
    (let [renderer (apple/create)
          invalid
          (record proto/patch-batch
                  (generation 1)
                  (ops [(proto/create-node-op 2 proto/Sheet)
                        (proto/set-prop-op 2 proto/Gap (proto/IntValue 8))]))]
      (is (thrown-with-msg?
           Invalid_argument
           #"unsupported property value|property is not supported|node properties conflict"
           ((:apply-batch (apple/backend renderer)) invalid))
          "a missing title and unsupported gap are rejected atomically")
      (assert-equal 0 (apple/node-count renderer)
...
(deftest flush-diagnostics-prove-local-signal-work
  (let [scheduler (sig/scheduler)
        renderer (apple/create)
        application (runtime/create scheduler (apple/backend renderer))
        component-scope (sig/scope "diagnostics")
        count-state (sig/state scheduler 0)
        root (runtime/create-node! application proto/Row)
        label (runtime/create-node! application proto/Text)
        label-value
    ...
    (let [diagnostics (runtime/diagnostics application)]
      (assert-equal runtime/Applied (:flush-status diagnostics)
                    "the initial batch is recorded as applied")
      (assert-equal 1 (:flush-generation diagnostics)
                    "diagnostics report the committed generation")
      (assert-equal 4 (:flush-operation-count diagnostics)
                    "the initial batch reports all retained operations")
      (assert-equal 2 (:flush-mounted-node-count diagnostics)
                    "diagnostics report retained node pressure")
      (assert-equal 0 (:flush-handler-count diagnostics)
                    "diagnostics report retained handler pressure")
      (assert-equal 0 (:flush-dynamic-segment-count diagnostics)
                    "diagnostics report dynamic segment pressure"))
    (sig/set! count-state 1)
    (runtime/flush! application)
    (let [diagnostics (runtime/diagnostics application)]
      (assert-equal runtime/Applied (:flush-status diagnostics)
                    "the local update is applied")
      (assert-equal 1 (:flush-operation-count diagnostics)
                    "one Signal update emits one retained property patch")
      (assert-equal 2 (:flush-signal-round-count diagnostics)
                    "state publication and derived mapping stay bounded")
      (assert-equal 0 (:flush-signal-effect-count diagnostics)
                    "the property update runs no effects")
      (assert-equal 2 (:flush-signal-dirty-task-count diagnostics)
                    "only the source and one derived Signal run")
      (assert-equal 2 (:flush-mounted-node-count diagnostics)
                    "local work preserves the retained tree"))
    (runtime/flush! application)
    (let [diagnostics (runtime/diagnostics application)]
      (assert-equal runtime/NoBatch (:flush-status diagnostics)
                    "a no-op flush is explicit")
      (assert-equal 0 (:flush-operation-count diagnostics)
                    "a no-op emits no retained operations")
      (assert-equal 0 (:flush-signal-round-count diagnostics)
                    "a no-op schedules no Signal round")
      (assert-equal 2 (:flush-generation diagnostics)
                    "a no-op preserves the committed generation"))))
...
(deftest rejected-platform-batch-is-not-committed
  (let [scheduler (sig/scheduler)
    ...
    (let [diagnostics (runtime/diagnostics application)]
      (assert-equal runtime/Rejected (:flush-status diagnostics)
                    "diagnostics expose the rejected backend boundary")
      (assert-equal 0 (:flush-generation diagnostics)
                    "a rejected batch does not advance generation")
      (assert-equal 1 (:flush-operation-count diagnostics)
                    "diagnostics retain the attempted operation count")
      (assert-equal 1 (:flush-pending-operation-count diagnostics)
                    "rejected operations remain available for diagnosis")
      (assert-equal 1 (:flush-mounted-node-count diagnostics)
                    "the LG retained node remains mounted"))))
