...
| `context-menu` | Right click, keyboard ContextMenu/Shift+F10, and 500 ms long press; cancel long press after more than 10 px movement; same menu behavior as Menu | Delay exists, but movement does not cancel and touch placement uses host bounds rather than the touch point | Track the initial touch, cancel after 10 px, open around a 10 px virtual anchor at the press point, and use the menu lifecycle. Always keep a visible primary route to the same actions. |
...
| `combobox` | Editable input with `aria-activedescendant`, filtered listbox, composition-safe input, arrows/Home/End/Enter/Escape, touch trigger handling, live status where needed | IME value patching is protected, and active descendant support exists, but popup lifecycle, collision behavior, empty state semantics, and touch tests are incomplete | Preserve the native input and composition buffer. Add the same popup lifecycle and input-modality tests; never replace editing with a contenteditable surface. |
...
| text inputs and textarea | Native editing, selection, clipboard, IME composition, mobile keyboard, and textarea auto-size | Composition and auto-size foundations exist | Retain native controls and add regression tests around Chinese composition inside Combobox and Sheet. Motion must never write the input value or recreate it. |
